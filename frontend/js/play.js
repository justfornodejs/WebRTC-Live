 ...
 *
 * 功能描述：
 *   实现浏览器端 WebRTC 拉流播放功能，低延迟接收来自推流端的媒体流
 *
 * 核心功能：
 *   - WebRTC 拉流：从 SRS 服务器接收实时媒体流
 *   - 连接管理：建立连接、状态监控、异常恢复
 *   - 实时统计：接收分辨率、帧率、码率等信息采集
 *   - 本地录制：支持拉流画面的录制与上传
 *
 * 技术架构：
 *   1. 接收轨道创建：添加 video/audio 轨道（方向为 recvonly）
 *   2. WebRTC 连接：创建 RTCPeerConnection、SDP 交换、ICE 候选收集
 *   3. 信令协商：通过 Flask 后端代理与 SRS 服务器交换 SDP
 *   4. 媒体接收：通过 WebRTC UDP 协议接收来自 SRS 的媒体流
 *   5. 本地播放：将接收流绑定到 video 元素
 *
 * 状态管理：
 *   - isPlaying: 拉流播放状态标志
 *   - playPC: RTCPeerConnection 实例
 *   - remoteVideo: 远端视频元素
 *   - playDuration: 播放时长（秒）
 *
 * 依赖模块：
